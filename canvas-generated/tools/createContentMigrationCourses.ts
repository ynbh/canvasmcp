...
import { tool } from "ai";
import { createContentMigrationCoursesDataSchema } from "./aitm.schema.ts";
import { createContentMigrationCourses, CreateContentMigrationCoursesData } from "..";
...
doing .zip file upload)
    `,
  parameters: createContentMigrationCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateContentMigrationCoursesData, "url"> ) => {
    try {
      const { data } = await createContentMigrationCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
