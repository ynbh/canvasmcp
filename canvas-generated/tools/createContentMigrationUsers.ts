...
import { tool } from "ai";
import { createContentMigrationUsersDataSchema } from "./aitm.schema.ts";
import { createContentMigrationUsers, CreateContentMigrationUsersData } from "..";
...
doing .zip file upload)
    `,
  parameters: createContentMigrationUsersDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateContentMigrationUsersData, "url"> ) => {
    try {
      const { data } = await createContentMigrationUsers(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
