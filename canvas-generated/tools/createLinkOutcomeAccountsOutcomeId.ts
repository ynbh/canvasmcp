...
import { tool } from "ai";
import { createLinkOutcomeAccountsOutcomeIdDataSchema } from "./aitm.schema.ts";
import { createLinkOutcomeAccountsOutcomeId, CreateLinkOutcomeAccountsOutcomeIdData } from "..";
...
ignored.
    `,
  parameters: createLinkOutcomeAccountsOutcomeIdDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateLinkOutcomeAccountsOutcomeIdData, "url"> ) => {
    try {
      const { data } = await createLinkOutcomeAccountsOutcomeId(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
