...
import { tool } from "ai";
import { createLinkOutcomeCoursesDataSchema } from "./aitm.schema.ts";
import { createLinkOutcomeCourses, CreateLinkOutcomeCoursesData } from "..";
...
ignored.
    `,
  parameters: createLinkOutcomeCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateLinkOutcomeCoursesData, "url"> ) => {
    try {
      const { data } = await createLinkOutcomeCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
