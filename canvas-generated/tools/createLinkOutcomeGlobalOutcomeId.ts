...
import { tool } from "ai";
import { createLinkOutcomeGlobalOutcomeIdDataSchema } from "./aitm.schema.ts";
import { createLinkOutcomeGlobalOutcomeId, CreateLinkOutcomeGlobalOutcomeIdData } from "..";
...
ignored.
    `,
  parameters: createLinkOutcomeGlobalOutcomeIdDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateLinkOutcomeGlobalOutcomeIdData, "url"> ) => {
    try {
      const { data } = await createLinkOutcomeGlobalOutcomeId(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
