...
import { tool } from "ai";
import { exportContentCoursesDataSchema } from "./aitm.schema.ts";
import { exportContentCourses, ExportContentCoursesData } from "..";
...
the exported content.
    `,
  parameters: exportContentCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<ExportContentCoursesData, "url"> ) => {
    try {
      const { data } = await exportContentCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
