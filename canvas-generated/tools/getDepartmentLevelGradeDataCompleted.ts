...
import { tool } from "ai";
import { getDepartmentLevelGradeDataCompletedDataSchema } from "./aitm.schema.ts";
import { getDepartmentLevelGradeDataCompleted, GetDepartmentLevelGradeDataCompletedData } from "..";
...
variations on endpoint as the participation data.
    `,
  parameters: getDepartmentLevelGradeDataCompletedDataSchema.omit({ url: true }),
  execute: async (args : Omit<GetDepartmentLevelGradeDataCompletedData, "url"> ) => {
    try {
      const { data } = await getDepartmentLevelGradeDataCompleted(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
