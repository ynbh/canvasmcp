...
import { tool } from "ai";
import { getDepartmentLevelParticipationDataCurrentDataSchema } from "./aitm.schema.ts";
import { getDepartmentLevelParticipationDataCurrent, GetDepartmentLevelParticipationDataCurrentData } from "..";
...
The action follows the suffix.
    `,
  parameters: getDepartmentLevelParticipationDataCurrentDataSchema.omit({ url: true }),
  execute: async (args : Omit<GetDepartmentLevelParticipationDataCurrentData, "url"> ) => {
    try {
      const { data } = await getDepartmentLevelParticipationDataCurrent(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
