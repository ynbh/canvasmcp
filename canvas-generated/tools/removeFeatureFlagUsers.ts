...
import { tool } from "ai";
import { removeFeatureFlagUsersDataSchema } from "./aitm.schema.ts";
import { removeFeatureFlagUsers, RemoveFeatureFlagUsersData } from "..";
...
flags that were masked by this one will apply again.
    `,
  parameters: removeFeatureFlagUsersDataSchema.omit({ url: true }),
  execute: async (args : Omit<RemoveFeatureFlagUsersData, "url"> ) => {
    try {
      const { data } = await removeFeatureFlagUsers(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
