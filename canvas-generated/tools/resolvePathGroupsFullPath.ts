...
import { tool } from "ai";
import { resolvePathGroupsFullPathDataSchema } from "./aitm.schema.ts";
import { resolvePathGroupsFullPath, ResolvePathGroupsFullPathData } from "..";
...
error is returned.
    `,
  parameters: resolvePathGroupsFullPathDataSchema.omit({ url: true }),
  execute: async (args : Omit<ResolvePathGroupsFullPathData, "url"> ) => {
    try {
      const { data } = await resolvePathGroupsFullPath(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
