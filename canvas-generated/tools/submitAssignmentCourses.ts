...
import { tool } from "ai";
import { submitAssignmentCoursesDataSchema } from "./aitm.schema.ts";
import { submitAssignmentCourses, SubmitAssignmentCoursesData } from "..";
...
Google Docs is not yet supported.
    `,
  parameters: submitAssignmentCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<SubmitAssignmentCoursesData, "url"> ) => {
    try {
      const { data } = await submitAssignmentCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
