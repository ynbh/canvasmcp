...
import { tool } from "ai";
import { uploadFileCoursesDataSchema } from "./aitm.schema.ts";
import { uploadFileCourses, UploadFileCoursesData } from "..";
...
assignment with these file ids.
    `,
  parameters: uploadFileCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<UploadFileCoursesData, "url"> ) => {
    try {
      const { data } = await uploadFileCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
