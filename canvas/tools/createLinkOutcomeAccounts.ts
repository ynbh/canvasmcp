...
import { tool } from "ai";
import { createLinkOutcomeAccountsDataSchema } from "./aitm.schema.ts";
import { createLinkOutcomeAccounts, CreateLinkOutcomeAccountsData } from "..";
...
ignored.
    `,
  parameters: createLinkOutcomeAccountsDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateLinkOutcomeAccountsData, "url"> ) => {
    try {
      const { data } = await createLinkOutcomeAccounts(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
