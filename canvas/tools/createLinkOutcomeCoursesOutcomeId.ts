...
import { tool } from "ai";
import { createLinkOutcomeCoursesOutcomeIdDataSchema } from "./aitm.schema.ts";
import { createLinkOutcomeCoursesOutcomeId, CreateLinkOutcomeCoursesOutcomeIdData } from "..";
...
ignored.
    `,
  parameters: createLinkOutcomeCoursesOutcomeIdDataSchema.omit({ url: true }),
  execute: async (args : Omit<CreateLinkOutcomeCoursesOutcomeIdData, "url"> ) => {
    try {
      const { data } = await createLinkOutcomeCoursesOutcomeId(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
