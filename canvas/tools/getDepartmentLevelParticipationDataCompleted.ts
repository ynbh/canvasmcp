...
import { tool } from "ai";
import { getDepartmentLevelParticipationDataCompletedDataSchema } from "./aitm.schema.ts";
import { getDepartmentLevelParticipationDataCompleted, GetDepartmentLevelParticipationDataCompletedData } from "..";
...
The action follows the suffix.
    `,
  parameters: getDepartmentLevelParticipationDataCompletedDataSchema.omit({ url: true }),
  execute: async (args : Omit<GetDepartmentLevelParticipationDataCompletedData, "url"> ) => {
    try {
      const { data } = await getDepartmentLevelParticipationDataCompleted(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
