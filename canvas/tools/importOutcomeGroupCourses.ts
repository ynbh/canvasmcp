...
import { tool } from "ai";
import { importOutcomeGroupCoursesDataSchema } from "./aitm.schema.ts";
import { importOutcomeGroupCourses, ImportOutcomeGroupCoursesData } from "..";
...
the root outcome group of its context.
    `,
  parameters: importOutcomeGroupCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<ImportOutcomeGroupCoursesData, "url"> ) => {
    try {
      const { data } = await importOutcomeGroupCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
