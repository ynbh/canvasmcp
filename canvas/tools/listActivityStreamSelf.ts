...
import { tool } from "ai";
import { listActivityStreamSelfDataSchema } from "./aitm.schema.ts";
import { listActivityStreamSelf, ListActivityStreamSelfData } from "..";
...
Returns
an {api:Submissions:Submission Submission} with its Course and Assignment
data.
...
1234
    ...
    `,
  parameters: listActivityStreamSelfDataSchema.omit({ url: true }),
  execute: async (args : Omit<ListActivityStreamSelfData, "url"> ) => {
    try {
      const { data } = await listActivityStreamSelf(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
