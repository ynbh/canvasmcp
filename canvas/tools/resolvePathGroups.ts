...
import { tool } from "ai";
import { resolvePathGroupsDataSchema } from "./aitm.schema.ts";
import { resolvePathGroups, ResolvePathGroupsData } from "..";
...
error is returned.
    `,
  parameters: resolvePathGroupsDataSchema.omit({ url: true }),
  execute: async (args : Omit<ResolvePathGroupsData, "url"> ) => {
    try {
      const { data } = await resolvePathGroups(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
