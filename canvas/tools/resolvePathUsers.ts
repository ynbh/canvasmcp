...
import { tool } from "ai";
import { resolvePathUsersDataSchema } from "./aitm.schema.ts";
import { resolvePathUsers, ResolvePathUsersData } from "..";
...
error is returned.
    `,
  parameters: resolvePathUsersDataSchema.omit({ url: true }),
  execute: async (args : Omit<ResolvePathUsersData, "url"> ) => {
    try {
      const { data } = await resolvePathUsers(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
