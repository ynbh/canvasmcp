...
import { tool } from "ai";
import { resolvePathUsersFullPathDataSchema } from "./aitm.schema.ts";
import { resolvePathUsersFullPath, ResolvePathUsersFullPathData } from "..";
...
error is returned.
    `,
  parameters: resolvePathUsersFullPathDataSchema.omit({ url: true }),
  execute: async (args : Omit<ResolvePathUsersFullPathData, "url"> ) => {
    try {
      const { data } = await resolvePathUsersFullPath(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
