...
import { tool } from "ai";
import { retrieveAssignmentsEnabledForGradeExportToSisCoursesDataSchema } from "./aitm.schema.ts";
import { retrieveAssignmentsEnabledForGradeExportToSisCourses, RetrieveAssignmentsEnabledForGradeExportToSisCoursesData } from "..";
...
information
    `,
  parameters: retrieveAssignmentsEnabledForGradeExportToSisCoursesDataSchema.omit({ url: true }),
  execute: async (args : Omit<RetrieveAssignmentsEnabledForGradeExportToSisCoursesData, "url"> ) => {
    try {
      const { data } = await retrieveAssignmentsEnabledForGradeExportToSisCourses(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
