...
import { tool } from "ai";
import { updateContentMigrationUsersDataSchema } from "./aitm.schema.ts";
import { updateContentMigrationUsers, UpdateContentMigrationUsersData } from "..";
...
start the process again.
    `,
  parameters: updateContentMigrationUsersDataSchema.omit({ url: true }),
  execute: async (args : Omit<UpdateContentMigrationUsersData, "url"> ) => {
    try {
      const { data } = await updateContentMigrationUsers(args);
      return data;
    } catch (e: unknown) {
      console.log(e);
      return "no results";
